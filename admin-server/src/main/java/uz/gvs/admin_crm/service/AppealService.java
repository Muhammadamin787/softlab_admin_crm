...
public class AppealService {
    ...
    public ApiResponse saveAppeal(AppealDto appealDto) {
        try {
            if (appealDto.getPhoneNumber() == null || appealDto.getFullName() == null || appealDto.getPhoneNumber().length() > 0
                    || appealDto.getFullName().length() > 0)
    ...
    */
    public ApiResponse getClientList(String enumType, Integer typeId, int page, int size) {
        try {
            ...
            if (typeId == 0) {
                clientList = clientRepository.getClientByFilterEnumType(enumType, page, size);
            } else {
            ...
            return apiResponseService.getResponse(clientList);
        } catch (Exception e) {
