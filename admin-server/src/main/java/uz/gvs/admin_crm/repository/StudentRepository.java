...
public interface StudentRepository extends JpaRepository<Student, UUID> {
    ...
    @Query(nativeQuery = true, value = "select cast(st.id as varchar ), ur.full_name from student st inner join users ur on st.user_id = ur.id inner join student_student_group ssg on st.id = ssg.student_id where :groupId<>any(select sg.group_id from student_group sg where ssg.student_group_id=sg.id) and  LOWER(ur.full_name) like concat('%', :objName, '%') or ur.phone_number like concat('%', :objName, '%')  limit 10")
