...
public interface StudentRepository extends JpaRepository<Student, UUID> {
    ...
    @Query(nativeQuery = true, value = "select cast(st.id as varchar), ur.full_name from student st inner join users ur on st.user_id = ur.id where LOWER(ur.full_name) like concat('%', :objName, '%') or ur.phone_number like concat('%', :objName, '%') limit 20")
